...
function problemA() {
  ...
  // promise version
  // ???
  let pro1 = promisifiedReadFile("poem-two/stanza-01.txt");
  let pro2 = promisifiedReadFile("poem-two/stanza-01.txt");

  let arrayPromesas = [pro1, pro2];
  Promise.all(arrayPromesas).then((lasPromesas) => {
    //  la devuelta es un arreglo con las dos promesas
    blue(lasPromesas[0]);
    blue(lasPromesas[1]);
    console.log("done");
  });
...
function problemB() {
  ...
  // promise version
  // ???
  let promesas = filenames.map((file) => promisifiedReadFile(file));
...
function problemC() {
  ...
  filenames
    .reduce((p, file) => {
      ...
      // el promise se coloca en falso para que  no me logee una promesa que no tiene nada
      // el then es para que me logee el ultimo
...
function problemD() {
  ...
  filenames
    ...
    .catch((error) => {
      if (error) magenta(new Error(error));
      console.log(error);
    });
}
